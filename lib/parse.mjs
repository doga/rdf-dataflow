...
function toDataset(dataflowObject) {
  try {
    ...
    subjectName   = jsonIn.head?.terms.subject ?? 's',
    predicateName = jsonIn.head?.terms.property ?? 'p',
    objectName    = jsonIn.head?.terms.object ?? 'o',
    graphName     = jsonIn.head?.terms.graph ?? 'g',
    dataset       = rdf.dataset();
    ...
    for (const binding of jsonIn.dataset) {
      const
...
function createSubjectTerm(term) {
  const res =  createQuad(term) || createNamedNode(term) || createBlankNode(term);
  if (!res) {
