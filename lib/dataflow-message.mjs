...
import { isArray, isPlainObject, isString, expandIri, isDefaultGraph } from "./util.mjs";
...
XSD_PREFIX = iri`http://www.w3.org/2001/XMLSchema#`,
XSD_STRING = iri`${XSD_PREFIX}string`,
...
class DataflowMessage {
  ...
  constructor(statements, config){
    ...
    if(!(isArray(statements) && statements.every(s => s instanceof Statement)))throw new TypeError('wrong statements');
  ...
  static fromObject(messageObject){
    ...
    try {
      ...
      // head
      let prefixes = isPlainObject(messageObject.head?.prefixes) ? messageObject.head?.prefixes : {};
      
      // body
      if (isArray(messageObject.prose)) {
        return new DataflowMessage(
          messageObject.prose.map(s => Statement.fromObject(s, prefixes)),
          {prefixes, isProse: true}
        );
      }
      if (isArray(messageObject.dataset)) {
        return new DataflowMessage(
          messageObject.dataset.map(s => Statement.fromObject(s, prefixes)),
          {prefixes, isProse: false}
        );
      }
      return null;
    } catch (error) {
...
class Statement {
  ...
  /**
   * Read statement from a dataflow js object
   * @param {Array} statement 
   * @param {Object} prefixes 
   * @returns {(Statement | null)}
   */
  static fromObject(statement, prefixes){
    if(!isArray(statement))return null;
    if(![3,4].includes(statement.length))return null;
    try {
      const
      subject   = NamedNode.fromObject(statement[0], prefixes) || BlankNode.fromObject(statement[0]) || Triple.fromObject(statement[0], prefixes),
      predicate = NamedNode.fromObject(statement[1], prefixes),
      object    = NamedNode.fromObject(statement[2], prefixes) || BlankNode.fromObject(statement[2]) || Literal.fromObject(statement[2], prefixes),
      graph     = NamedNode.fromObject(statement[3], prefixes) || BlankNode.fromObject(statement[3]);
...
class Triple {
  ...
  constructor(subject, predicate, object){
    if (!(
      [NamedNode, BlankNode].some(termType => subject instanceof termType) &&
      predicate instanceof NamedNode &&
      [NamedNode, BlankNode, Literal].some(termType => object instanceof termType) 
    )) throw new TypeError('wrong term');
    this.#subject   = subject;
    this.#predicate = predicate;
    this.#object    = object;
  }
  ...
  /**
   * Read triple from a dataflow js object
   * @param {Object} triple 
   * @param {Object} prefixes 
   * @returns {(Triple | null)}
   */
  static fromObject(triple, prefixes){
    if(!isPlainObject(triple))return null;
    if(triple.type !== 'triple')return null;
    if(!isArray(triple.value))return null;
    try {
      const
      subject   = NamedNode.fromObject(triple.value[0], prefixes) || BlankNode.fromObject(triple.value[0]),
      predicate = NamedNode.fromObject(triple.value[1], prefixes),
      object    = NamedNode.fromObject(triple.value[2], prefixes) || BlankNode.fromObject(triple.value[2]) || Literal.fromObject(triple.value[2], prefixes);
...
class NamedNode {
  ...
  /**
   * Read named node from a dataflow js object
   * @param {Object} namedNode 
   * @param {Object} prefixes 
   * @returns {(NamedNode | null)}
   */
  static fromObject(namedNode, prefixes){
    if(!isPlainObject(namedNode))return null;
    if(namedNode.type !== 'iri')return null;
    if(!isString(namedNode.value))return null;
    try {
      return new NamedNode(iri`${expandIri(namedNode.value, prefixes)}`);
    } catch (error) {
      return null;
    }
  }

  toObject(prefixes, isDatatype){
...
class BlankNode {
  ...
  /**
   * Read blank node from a dataflow js object
   * @param {Object} blankNode 
   * @returns {(BlankNode | null)}
   */
  static fromObject(blankNode){
    if(!isPlainObject(blankNode))return null;
    if(blankNode.type !== 'bnode')return null;
    try {
      return new BlankNode(blankNode.value);
    } catch (error) {
      return null;
...
class Literal {
  ...
  */
  ...
  get directionIsLtr(){return this.#directionIsLtr;}
  get datatype(){return this.#datatype;}

  /**
   * 
   * @param {string} value 
   * @param {(LanguageTag | undefined | null)} language 
  ...
  constructor(value, language, directionIsLtr, datatype){
    ...
    if(!(language instanceof LanguageTag)) language = null;
  ...
  static fromRDF(literal){
    ...
    try {
      ...
      const lang = langTag`${literal.language}`;
  ...
  /**
   * Read literal from a dataflow js object
   * @param {Object} literal 
   * @param {Object} prefixes 
   * @returns {(Literal | null)}
   */
  static fromObject(literal, prefixes){
    try {
      if(!isPlainObject(literal))return null;
      if(literal.type !== 'literal')return null;
      if(!isString(literal.value))return null;
      
      // literal has language?
      if (isString(literal.language)) {
        const lang = langTag`${literal.language}`;
        if(!lang)return null;
        if (isString(literal.direction)) {
          if (!['ltr', 'rtl'].includes(literal.direction))return null;
          return new Literal(literal.value, lang, literal.direction === 'ltr');
        }
        return new Literal(literal.value, lang);
      }
      
      // literal has datatype?
      if (isString(literal.datatype)) {
        const datatype = new NamedNode(iri`${expandIri(literal.datatype, prefixes)}`); // if(!datatype)return null;
        return new Literal(literal.value, null, null, datatype);
      }
      
      // literal is plain string
  ...
  toObject(prefixes){
    ...
    } else if(!XSD_STRING.equals(iri`${this.#datatype.value}`)) {
      res.datatype = this.#datatype.toObject(prefixes, true);
    }
    return res;
  }
