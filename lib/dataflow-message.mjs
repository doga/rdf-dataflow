...
class DataflowMessage {
  ...
  print(includeConfig){
    console.group('Dataflow message:');
...
class Statement {
  ...
  toObject(){
    const res = [this.#subject.toObject(), this.#predicate.toObject(), this.#object.toObject()];
    if(this.#graph)res.push(this.#graph.toObject());
    return res;
  }
...
class Triple {
  ...
  toObject(){
    return {
      type: 'triple',
      value: [
        this.#subject.toObject(),
        this.#predicate.toObject(),
...
class NamedNode {
  ...
  toObject(){
    return {type: 'iri', value: this.#value};
  }
  ...
  */
  ...
  static fromRDF(literal){
    ...
    try {
      ...
      return new Literal(
        literal.value, 
        lang, 
        lang ? (literal.direction === 'ltr') : null,
  ...
  toObject(){
    ...
    if(this.#language){
      ...
      if(!this.#directionIsLtr)res.direction = 'rtl';
    } else {
      res.datatype = this.#datatype.toObject();
    }
    return res;
  }
